...
class EC_Management {
  ...
  /**
	...
	*/
	 ...
	 */
	function addEvent($title, $location, $linkout, $description, $startDate, $startTime, $endDate, $endTime, $accessLevel, $postID, $eventID) {          
		$this->db->addEvent($title, $location, $linkout, $description, $startDate, $startTime, $endDate, $endTime, $accessLevel, $postID, $eventID);
		return;
	}
...
<?php
    ...
    <?php
   	...
   	/**
	 * Add CiviCRM Events to the Calendar.
	 */
	function addCiviCRMEvents() {  
     ?>
     <div id="icon-edit-pages" class="icon32"></div>
    <div class="wrap">     
    <h2>Add CiviCRM Events(S)</h2> 
     <?php
      ...
      $events = civicrm_api("Event","get", array (version => '3','page' =>'CiviCRM', 'q' =>'civicrm/ajax/rest', 'sequential' =>'1'));
      $eventsAdded = array();
      $i=0; 
      foreach($events['values'] as $eventDetails => $event) {       
        $linkout = get_bloginfo('url').'?page=CiviCRM&q=civicrm/event/register&id='.$event['id'].'&reset=1';      
        $startDetails = explode(' ', $event['start_date']);
        $endDetails = explode(' ', $event['end_date']);  
        $sql = "SELECT * FROM wp_eventscalendar_main WHERE `eventId` =".$event['id']; 
        global $wpdb;
        $results =  $wpdb->get_results($sql);       
        if(empty($results)) {  		 
            $this->addEvent($event['title'], '', $linkout, $event['description'], $startDetails[0], $startDetails[1], $endDetails[0], $endDetails[1], 'public',null, $event['id']);  
            $eventsAdded[$i] = $event['id'] ; 
            $i++;
        }
      }
      $eventsAdded;         
     ?> 
    <span><p> <?php echo count($eventsAdded); ?>  CiviCRM Events Added to the Calendar</p></span>
